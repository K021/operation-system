...
int ts;  // number of time slice to run
linkedList *rq;  // ready queue
int nvs[5];  // nice values
int nvs_start_idx = 0;
node *running_node;  // node of running process
double weight[5] = {0.64, 0.8, 1, 1.25, 1.5625};

node* createNode(char letter, pid_t pid, int nice, double vruntime) {
    node *nd = (node *)malloc(sizeof(node));
    pcb *psdata = (pcb *)malloc(sizeof(pcb));
    psdata->letter = letter;
    psdata->pid = pid;
    psdata->nice = nice;
...
void addNode(linkedList *l, char letter, pid_t pid, int nice, double vruntime) {
    node *newNode = (node *)malloc(sizeof(node));
    newNode = createNode(letter, pid, nice, vruntime);
...
node* popHeadNode(linkedList *l) {
    ...
    l->head = curr->next;
    if(curr->next) curr->next->fore = NULL;
    curr->next = NULL;
    l->len--;
    return curr;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < nps; ++i) {
        ...
        if (pid == 0) {
            ...
            addNode(rq, letter_printed, pid, getNice(), 0);
        }
    }
